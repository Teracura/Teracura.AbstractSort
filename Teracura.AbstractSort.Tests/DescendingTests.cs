...
public class DescendingTests
{
    ...
    {
        var config = new SortConfig<int>.Builder().MutateOriginal().Ascending(false).Mode(SortMode.Length)
    ...
    {
        var config = new SortConfig<int>.Builder().MutateOriginal().Ascending(false).Mode(SortMode.Length)
            .ReturnType(ReturnType.HashSet).Build();
        config.Ascending.ShouldBeFalse();
    ...
    {
        var config = new SortConfig<int>.Builder().MutateOriginal().Ascending(false).Mode(SortMode.Length)
            .ReturnType(ReturnType.Stack).Build();
        config.Ascending.ShouldBeFalse();
